    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-xl border-b border-border/50">
        <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            ...
            <Link href="/trending-hashtags-turkey" className="text-sm text-primary font-medium">Trend Hashtag&apos;ler</Link>
      ...
      <section className="relative pt-28 pb-12 px-6 overflow-hidden">
        ...
        <div className="max-w-6xl mx-auto text-center relative z-10">
          ...
          >
            Turkiye&apos;de{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal to-primary">
          ...
          >
            ...
            <Link href="/tiktok-trend-report" className="inline-flex items-center gap-1.5 text-xs text-primary hover:text-primary/80 transition-colors">
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center gap-3">
          <div className="relative flex-1 max-w-md w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 bg-card border border-border rounded-xl text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-teal/50"
            />
          </div>
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${
                  categoryFilter === cat
                    ...
                    : "bg-card border-border text-muted-foreground hover:border-teal/20"
                }`}
              >
                {cat}
              </button>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          {loading ? (
            ...
            <div className="bg-card border border-border rounded-xl overflow-hidden">
              {/* Header */}
              <div className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-border text-[10px] text-muted-foreground uppercase tracking-wider">
              ...
              {freeHashtags.map((hashtag, i) => (
                <motion.div
                  ...
                  className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-border/50 hover:bg-muted/50 transition-colors items-center"
                >
                  ...
                  <div className="col-span-3 flex items-center gap-2">
                    ...
                    {hashtag.isEmerging && (
                      <span className="text-[9px] bg-primary/10 text-primary px-1.5 py-0.5 rounded">Yukselen</span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2 text-right">
                    <span className="text-xs text-foreground font-medium flex items-center justify-end gap-1">
                      <Eye className="w-3 h-3 text-muted-foreground" />
                      {formatNumber(hashtag.totalUses)}
                    </span>
                  </div>
                  <div className="col-span-2 text-right">
                    <span className="text-xs text-foreground">{formatNumber(hashtag.avgViews)}</span>
                  </div>
                  <div className="col-span-2 text-right">
                    <span className={`text-xs font-medium flex items-center justify-end gap-0.5 ${
                      hashtag.weeklyGrowth > 0 ? "text-teal" : "text-primary"
                    }`}>
                      {hashtag.weeklyGrowth > 0 ? <ArrowUp className="w-3 h-3" /> : <ArrowDown className="w-3 h-3" />}
        ...
        <section className="px-6 pb-16">
          <div className="max-w-6xl mx-auto">
            ...
            <div className="relative">
              <div className="bg-card border border-border rounded-xl overflow-hidden blur-sm opacity-50 pointer-events-none select-none">
                {premiumHashtags.slice(0, 8).map((hashtag, i) => (
                  <div key={hashtag.id} className="grid grid-cols-12 gap-4 px-4 py-3 border-b border-border/50 items-center">
                    <div className="col-span-1 text-xs text-muted-foreground">{i + 16}</div>
                    <div className="col-span-3 text-sm text-foreground">{hashtag.name}</div>
                    <div className="col-span-2 text-xs text-muted-foreground">{hashtag.category}</div>
                ...
                <div className="bg-card/95 backdrop-blur-sm border border-primary/30 rounded-2xl p-8 text-center max-w-md">
                  ...
                  <ul className="text-left space-y-2 mb-6">
                    <li className="flex items-center gap-2 text-xs text-muted-foreground">
                      <Hash className="w-3 h-3 text-teal shrink-0" /> Tam hashtag veritabani
                    </li>
                    <li className="flex items-center gap-2 text-xs text-muted-foreground">
                      <BarChart2 className="w-3 h-3 text-teal shrink-0" /> Detayli trend grafikleri
                    </li>
                    <li className="flex items-center gap-2 text-xs text-muted-foreground">
                      <TrendingUp className="w-3 h-3 text-teal shrink-0" /> Buyume tahmini
                    </li>
                    <li className="flex items-center gap-2 text-xs text-muted-foreground">
                      <FileText className="w-3 h-3 text-teal shrink-0" /> Rekabet analizi
                    </li>
                  </ul>
                  <Link
                    href="/register"
      ...
      <section className="px-6 py-16 bg-card/50">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="prose prose-invert prose-sm max-w-none space-y-4 text-muted-foreground">
            ...
            <h3 className="text-lg font-semibold text-foreground">Dogru Hashtag Nasil Secilir?</h3>
          ...
          <div className="mt-10 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Link href="/tiktok-trend-report" className="bg-card border border-border rounded-xl p-5 hover:border-primary/30 transition-all group">
              <div className="flex items-center gap-3 mb-2">
      ...
      <section className="px-6 py-16">
        <div className="max-w-3xl mx-auto text-center">
          <div className="bg-gradient-to-br from-teal/10 via-card to-primary/10 border border-border rounded-2xl p-10">
