...
export default function ContactPage() {
  ...
  const categories: { value: Category; label: string; icon: React.ReactNode; desc: string }[] = [
    { value: "destek", label: "Destek", icon: <FiHelpCircle className="w-4 h-4" />, desc: "Hesap veya platform ile ilgili yardim" },
    { value: "hata", label: "Hata Bildirimi", icon: <FiAlertCircle className="w-4 h-4" />, desc: "Bug veya teknik sorun bildirin" },
    { value: "oneri", label: "Oneri", icon: <FiMessageCircle className="w-4 h-4" />, desc: "Yeni ozellik veya iyilestirme onerisi" },
    { value: "diger", label: "Diger", icon: <FiMail className="w-4 h-4" />, desc: "Genel sorular ve is birligi" },
  ];
          ...
          {submitted ? (
            ...
            >
              <div className="w-16 h-16 rounded-2xl bg-teal/10 border border-teal/20 flex items-center justify-center mx-auto mb-6">
                <FiCheckCircle className="w-8 h-8 text-teal" />
              </div>
              ...
              <Link
                href="/"
                className="inline-flex items-center gap-2 bg-surface border border-border text-text-primary px-6 py-2.5 rounded-xl hover:border-neon-red/30 transition-all text-sm font-medium"
              >
                Ana Sayfaya Don
              </Link>
            </motion.div>
            ...
            <>
              ...
              >
                ...
                <h1 className="text-3xl md:text-4xl font-bold mb-3">
                  Size Nasil{" "}
                  <span className="text-neon-red">Yardimci</span>{" "}
                  Olabiliriz?
                </h1>
              ...
              >
                {/* Category selection */}
                <div>
                  <label className="text-xs font-medium text-text-secondary mb-3 block">Konu</label>
                      ...
                      <button
                        ...
                        className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all text-center ${
                          category === cat.value
                            ...
                            : "bg-surface border-border text-text-secondary hover:border-neon-red/20"
                        }`}
                      >
                        {cat.icon}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-medium text-text-secondary mb-1.5 block">Adiniz</label>
                    <input
                      type="text"
                      value={name}
                      ...
                      required
                      className="w-full px-4 py-2.5 bg-surface border border-border rounded-xl text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-neon-red/50 transition-colors"
                    />
                  </div>
                  <div>
                ...
                {/* Message */}
                <div>
                  <label className="text-xs font-medium text-text-secondary mb-1.5 block">Mesajiniz</label>
                  <textarea
                    value={message}
                    ...
                    required
                    rows={5}
                    className="w-full px-4 py-3 bg-surface border border-border rounded-xl text-sm text-text-primary placeholder:text-text-muted focus:outline-none focus:border-neon-red/50 transition-colors resize-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 bg-neon-red text-white py-3 rounded-xl font-medium text-sm hover:bg-neon-red-light transition-all disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-lg hover:shadow-neon-red/20"
                >
                  {sending ? (
              ...
              <motion.div
                ...
                className="mt-12 bg-surface border border-border rounded-xl p-6"
              >
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <details key={faq.q} className="group">
                      <summary className="text-xs text-text-primary cursor-pointer hover:text-neon-red transition-colors list-none flex items-center gap-2">
                        <FiHelpCircle className="w-3 h-3 text-text-muted shrink-0" />
                        {faq.q}
                      </summary>
                      <p className="text-xs text-text-muted mt-1 ml-5">{faq.a}</p>
                    </details>
                  ))}
                </div>
              </motion.div>
