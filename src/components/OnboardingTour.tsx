  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          <div className="bg-surface/95 backdrop-blur-md border border-border/60 rounded-xl shadow-lg overflow-hidden">
            {/* Header */}
              ...
              >
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-xs font-bold text-text-primary mb-0.5">
                      {step.title}
                    </h3>
            ...
            <div className="px-3.5 pb-2.5 pt-0.5 flex items-center justify-between">
              ...
              <div className="flex items-center gap-1.5">
                {current > 0 ? (
                  <button
                    onClick={prev}
                    className="flex items-center gap-0.5 text-[10px] text-text-muted hover:text-text-primary transition-colors px-1.5 py-1 rounded"
                  >
                    <FiArrowLeft className="w-2.5 h-2.5" />
                    Geri
                  </button>
                ) : (
                  <button
                    onClick={dismiss}
                    className="text-[10px] text-text-muted hover:text-text-primary transition-colors px-1.5 py-1 rounded"
                  >
                    Gec
                  </button>
                )}
                <button
                  onClick={next}
                  className="flex items-center gap-1 text-[10px] font-medium bg-neon-red/90 text-white px-2.5 py-1 rounded-md hover:bg-neon-red transition-colors"
                >
                  {isLast ? (
                    <>
                      <FiCheck className="w-2.5 h-2.5" /> Tamam
                    </>
                  ) : (
                    <>
                      Sonraki <FiArrowRight className="w-2.5 h-2.5" />
                    </>
                  )}
                </button>
              </div>
